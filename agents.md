...
-- 3) 원본 카테고리 -> 핫토리 카테고리 N:1 매핑
create table public.category_mappings (
  source_category_id  bigint primary key
...
'원본 카테고리(source_categories) → 핫토리 카테고리(categories) 매핑 (N:1)';

comment on column public.category_mappings.source_category_id is
'PK이자 FK: source_categories.id (각 원본 카테고리는 하나의 핫토리 카테고리에만 매핑됨)';

comment on column public.category_mappings.category_id is
'FK: categories.id (여러 source_category가 동일 category로 매핑 가능)';

comment on column public.category_mappings.created_at is
'생성 시각';


-- 3-1) 카테고리 매핑 현황 한눈에 보기
select
  sc.source,
  sc.name as source_category,
  c.name  as hotori_category
from category_mappings cm
join source_categories sc on sc.id = cm.source_category_id
join categories c on c.id = cm.category_id
order by sc.source, sc.name;


category_mappings는 ID 기반 관계 테이블이며,
사람이 확인할 때는 v_category_mappings 뷰를 사용한다.
...
create table public.deals (
  ...
  title              text not null,
  shop_name          text,                   -- 쇼핑몰명(대표)
  subcategory        text,                   -- 서브카테고리(해석 결과)
...
comment on column public.deals.shop_name is '대표 쇼핑몰명';
comment on column public.deals.subcategory is '서브카테고리(해석 결과)';
...
comment on column public.deals.subcategory is '하위 카테고리';
comment on column public.deals.shop_name is '핫딜 링크의 쇼핑몰명';
